Prism.languages.haskell= {
	...
	'char': /'([^\\"]|\\([abfnrtv\\"'&]|\^[A-Z@[\]\^_]|NUL|SOH|STX|ETX|EOT|ENQ|ACK|BEL|BS|HT|LF|VT|FF|CR|SO|SI|DLE|DC1|DC2|DC3|DC4|NAK|SYN|ETB|CAN|EM|SUB|ESC|FS|GS|RS|US|SP|DEL|\d+|o[-7]+|x[-9a-fA-F]+))'/g,
	'string': /"([^\\"]|\\([abfnrtv\\"'&]|\^[A-Z@[\]\^_]|NUL|SOH|STX|ETX|EOT|ENQ|ACK|BEL|BS|HT|LF|VT|FF|CR|SO|SI|DLE|DC1|DC2|DC3|DC4|NAK|SYN|ETB|CAN|EM|SUB|ESC|FS|GS|RS|US|SP|DEL|\d+|o[-7]+|x[-9a-fA-F]+)|\\\s+\\)*"/g,
	...
	'import_statement' : {
		...
		pattern: /(\n|^)\s*(import)\s+(qualified\s+)?(([A-Z][_a-zA-Z-9']*)(\.[A-Z][_a-zA-Z-9']*)*)(\s+(as)\s+(([A-Z][_a-zA-Z-9']*)(\.[A-Z][_a-zA-Z-9']*)*))?(\s+hiding\b)?/gm,
		inside: {
	...
	'number' : /\b(\d+(\.\d+)?([eE][+-]?\d+)?|[Oo][-7]+|[Xx][-9a-fA-F]+)\b/g,
	// Most of this is needed because of the meaning of a single '.'.
	// If it stands alone freely, it is the function composition.
	// It may also be a separator between a module name and an identifier => no
	// operator. If it comes together with other special characters it is an
	// operator too.
	'operator' : /\s\.\s|([-!#$%*+=\?&@|~:<>^\\]*\.[-!#$%*+=\?&@|~:<>^\\]+)|([-!#$%*+=\?&@|~:<>^\\]+\.[-!#$%*+=\?&@|~:<>^\\]*)|[-!#$%*+=\?&@|~:<>^\\]+|(`([A-Z][_a-zA-Z-9']*\.)*[_a-z][_a-zA-Z-9']*`)/g,
	// In Haskell, nearly everything is a variable, do not highlight these.
	'hvariable': /\b([A-Z][_a-zA-Z-9']*\.)*[_a-z][_a-zA-Z-9']*\b/g,
	'constant': /\b([A-Z][_a-zA-Z-9']*\.)*[A-Z][_a-zA-Z-9']*\b/g,
	'punctuation' : /[{}[\];(),.:]/g
};
